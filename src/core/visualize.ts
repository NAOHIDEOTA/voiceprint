 ...
 */
export function voiceprintToHeatmap(
  voiceprint: Voiceprint,
  options?: { cols?: number; normalize?: boolean },
): number[][] {
 ...
 */
export function spectrogramToGrid(
  flat: number[],
  frames: number,
  bins: number,
): number[][] {
  ...
  if (flat.length !== frames * bins) {
    throw new Error(
      `spectrogram shape mismatch: ${flat.length} values for ${frames}x${bins}`,
    );
  }
