...
function fail(code: VoiceprintErrorCode, message: string, details?: unknown): ExtractResult {
...
export async function extractVoiceprint(
  ...
  if (pre.speechMs < minSpeechMs) {
    return fail(
      'SPEECH_TOO_SHORT',
      `detected speech is too short: ${pre.speechMs}ms < ${minSpeechMs}ms`,
      { speechMs: pre.speechMs, minSpeechMs },
    );
  }
  ...
  try {
    ...
    vector = emb.data as Float32Array;
  } catch (e) {
  ...
  if (pre.clipping) warnings.push('audio contains clipping');
  ...
  // 7. 可視化
  let visualization: VoiceVisualization | undefined;
